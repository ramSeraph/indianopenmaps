...
def get_metadata(mosaic_data):
    ...
    if len(vector_layers) > 1:
        raise Exception('only raster or one vector layer is supported')
    for k, item in mosaic_data.items():
        h = item['header']
        # TODO: not generic, max and min zoom can come from metadata
    ...
    center_zoom = 9 # TODO: not generic
    cm['center'] = f"{center_lon},{center_lat},{center_zoom}"
    if len(vector_layers) != 0:
        vector_layers[0]['minzoom'] = cm['minzoom']
        vector_layers[0]['maxzoom'] = cm['maxzoom']
...
if __name__ == '__main__':
    import sys
    usage = f'{sys.argv[0]} <mosaic_url> <output_file>'
    if len(sys.argv) != 3:
        print('ERROR: unexpected number of arguments')
        print(f'USAGE: {usage}')
        exit(1)

    mosaic_url = sys.argv[1]
    out_mbtiles_file = sys.argv[2]
