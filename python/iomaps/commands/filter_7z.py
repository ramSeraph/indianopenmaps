...
@click.option("--completely-inside", is_flag=True, help="Only pick features that are completely inside the filtering shape.")
...
def filter_7z(input_7z, output_file, log_level, filter_file, filter_file_driver, bounds, no_clip, completely_inside, schema_file, limit_to_geom_type, strict_geom_type_check, output_driver, pick_filter_feature_id, pick_filter_feature_kv):
    """
    ...
    filter = create_filter(filter_file=filter_file, filter_file_driver=filter_file_driver, bounds=bounds, no_clip=no_clip, completely_inside=completely_inside, limit_to_geom_type=limit_to_geom_type, strict_geom_type_check=strict_geom_type_check, pick_filter_feature_id=pick_filter_feature_id, pick_filter_feature_kv=pick_filter_feature_kv)
    if filter is None:
        raise click.Abort() 
